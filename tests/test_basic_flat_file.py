...
with open(FOLDER.parent / "tax_microdata_benchmarking" / "taxcalc_variable_metadata.yaml") as f:
...
def test_tc_variable_totals(variable):
    ...
    if tc_variable_totals[variable] == 0:
        ...
        return
    assert abs(total / tc_variable_totals[variable] - 1) < 0.5, f"{variable} ({name}) is off by {total / tc_variable_totals[variable] - 1:.1%}"
