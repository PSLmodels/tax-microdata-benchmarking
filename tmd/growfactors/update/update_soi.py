...
def update_soi(year, datapath, wage_indicies, file_):
    ...
    wage_indicies: wage bin index pairs; see PUFWAGES
    file_: "cps" or "puf"
    """
...
def table12(year, datapath):
    ...
    col = (
        "Returns of married persons filing jointly and returns of "
        "surviving spouses"
    )
...
def main():
    """
    Add a year of SOI estimates to data/SOI_estimates.csv.
    """
    ...
    # NOTE: TaxData also updated cps_stage1/SOI_estimates.csv here; TMD
    # uses only the PUF-based estimates, so that half has been dropped.
    puf_vals = update_soi(args.year, args.path, PUFWAGES, "puf")
    pufpath = DATA_FOLDER / "SOI_estimates.csv"
    puf_soi = pd.read_csv(pufpath, index_col=0)
    puf_soi[args.year] = puf_vals
