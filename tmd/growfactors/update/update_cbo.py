...
# requests_html and jinja2 are installed only by the optional
# "growfactors-update" extra; see tmd/growfactors/docs/annual_update.md
from requests_html import HTMLSession  # pylint: disable=import-error
from jinja2 import Template  # pylint: disable=import-error

CUR_PATH = Path(__file__).resolve().parent
DATA_FOLDER = CUR_PATH.parent / "data"
DOCS_FOLDER = CUR_PATH.parent / "docs"
...
def update_econproj(url, baseline, text_args):
    ...
    # increase the CBO final year to (the last year + 1) for each
    # update.  e.g. when the CBO final year from CBO is 2033, make
    # the update as range(2017, 2035 + 1)
...
def update_cbo():
    out_path = DOCS_FOLDER / "CBO_Baseline_Updating_Instructions.md"
    template_str = (DOCS_FOLDER / "cbo_instructions_template.md").open().read()
    current_text = out_path.open().read()
    text_args = fill_text_args(current_text)
    baseline = pd.read_csv(DATA_FOLDER / "CBO_baseline.csv", index_col=0)
    ...
    RETS_URL = (
        ...
        "soi-tax-stats-calendar-year-projections-publication-6187"
    )
    UCOMP_URL = (
...
def main():
    """
    Update CBO_baseline.csv and the CBO updating instructions in place.
    """
    update_cbo().to_csv(DATA_FOLDER / "CBO_baseline.csv")
    return 0
