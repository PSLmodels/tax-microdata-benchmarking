...
The quarto chunk `cdpop-download` will, if a user sets the chunk `eval` option to `eval: true`, download and save Congressional District population data from the American Community Survey (ACS) for 2021 and for the 5-year period ending in 2021, based on the 116th Congress.

Ordinarlily this will not be necessary because previously downloaded files are included with the project in the "../cds/raw_data" folder. Thus, the default chunk option is `eval: false` and the chunk will not be run when this project is rendered.
