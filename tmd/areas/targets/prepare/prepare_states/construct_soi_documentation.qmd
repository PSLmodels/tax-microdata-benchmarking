...
count(cleaned1 |> 
        filter(str_starts(basevname, "18")),
      vname, udescription, year)

# salt variables basevname 18400, sum of 18425 and 18450, both avail 2015-2021
newvars_base <- read_delim(
"basevname; description_base
...
newvars <- newvars_base |>
  ...
  mutate(
    vname=case_when(
      vtype == "amount" ~ paste0("a", basevname),
      vtype == "count" ~ paste0("n", basevname),
      .default = "ERROR"),
    description=case_when(
      vtype == "amount" ~ paste0(str_to_sentence(description_base), " amount"),
      vtype == "count" ~ paste0("Number of returns with ", description_base),
    .default = "ERROR"),
