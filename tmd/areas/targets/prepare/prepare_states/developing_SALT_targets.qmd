...
We want *state-weighted* TMD microdata files to reflect *potential* SALT deductions in individual states, including nonitemizers as well as itemizers, and before application of any caps. This is what our SALT targets should reflect.

IRS publishes state-by-state *actual* SALT deductions, not *potential* deductions. Actual SALT deductions are sharply reduced from earlier years due to the drop in itemization caused by the TCJA standard deduction increase.

This section discusses how we develop state targets for potential SALT deductions using the published IRS data on actual SALT deductions by state.

(Note that the same issue arises for Congressional districts, where we take a similar approach.)
