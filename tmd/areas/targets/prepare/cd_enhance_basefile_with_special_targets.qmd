...
socsec_final <- socseccd2 |> 
  right_join(socsecus_long |> 
             ...
             by = join_by(agistub, vname),
             relationship = "many-to-many") |> 
  mutate(target=socsecus *  soishare)
head(socsec_final)
...
socsecus_long |> filter(data_source==1) |> summarise(socsecus=sum(socsecus), .by=c(vname, tmdvname))
...
stack_enhanced <- stack |> 
  ...
  bind_rows(salt_final |> select(all_of(names(stack))))
