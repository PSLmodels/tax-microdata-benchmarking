...
-   After checking the above and starting RStudio, in the console run `renv::restore()` and answer y when asked whether to install packages. This will take a while.

`renv` is a kind of package manager for R that can set up a project-specific environment that is the same across multile machines. It should ensure that your environment includes needed versions of R packages used in this project. (Most packages are loaded in ".../targets/prepare/R/libraries.R".)
