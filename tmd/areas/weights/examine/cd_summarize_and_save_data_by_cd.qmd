...
tmdplusweights <- tmd2021 |> 
  select(RECID, data_source, us=s006, all_of(taxcalc_vars)) |> 
...
tmdplusweights[1:5, c(1:12, ncol(tmdplusweights))]
...
wtdsums <- long1 |> 
  summarise(across(us:wy00,
                \(x) sum(x * value)),
            .by=c(irange, data_source, variable)) |> 
  pivot_longer(-c(irange, data_source, variable),
               names_to = "statecd",
               values_to = "sum")
