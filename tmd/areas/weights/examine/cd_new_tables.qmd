...
wtdsums1 <- wtdsums |> 
  rename(fstatus=MARS) |> 
  mutate(variable=ifelse(variable=="wtdn", "XTOT", variable),
         scope=case_when(variable=="XTOT" ~ 0,
                         data_source==1 ~ 1,
                         data_source==0 ~ 2,
                         .default=-999999),
         count=0,
         agistub=as.integer(irange))
count(wtdsums1, irange, agistub) |> 
  arrange(irange)
...
combo <- targets_used |> 
  rename(statecd=src) |> 
  left_join(wtdsums1,
            by = join_by(statecd, count, scope, fstatus))
