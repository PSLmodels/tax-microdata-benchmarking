...
# Congressional District data preparation
...
This section prepares a comparison file with the following information:

-   weighted sums for each area for each of the following TMD variables, for each AGI range, filer / non-filer indicator, and filing status:
       `r paste(TAXCALC_VARS, collapse=", ")`
-   target values for the variable, if available, based on SOI or other appropriate data
-   difference and percentage difference between the weighted sum and target value (if available)
-   an indicator for whether the variable was targeted or instead "floated free"

It saves the file as compfile.csv in the `data_cd` or `data_state` folder as appropriate.
