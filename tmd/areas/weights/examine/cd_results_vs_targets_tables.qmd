...
## Show results vs. targets (TO COME)

Units:

-   Amounts are in $ millions (varname==amount for target, wtdvalue, and diff)
-   Counts are actual numbers
...
compfile |>
  ...
  mutate(across(c(target, wtdvalue, diff), 
                \(x) ifelse(vartype=="amount", x / 1e6, x))) |> 
