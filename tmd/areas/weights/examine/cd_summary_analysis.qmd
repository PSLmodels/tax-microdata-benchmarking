...
tabdata |> 
  ...
  tab_spanner(
    md("Cumulative number of items by targeting status and percentage-error group"),
  ...
  cols_label(apcut="Error-group bounds (e.g., (0.005,0.01] = > 0.5% and <= 1% error bin",
...
## Quantiles of percentage differences
...
tabdata <- compfile |>
  summarise(pdiff=list(quantile(abs(pdiff),
                                probs=probs)),
            .by=c(targeted)) |>
  mutate(targeted=ifelse(targeted, "targeted", "not_targeted")) |> 
