...
files |> 
  ...
  # walk(~ drive_download(as_id(.x), overwrite = TRUE))
...
targets1 <- vroom(targetfiles, id="src") |> 
...
glimpse(targets1)
count(targets1, src)
count(targets1, active)
count(targets1, varname)
count(targets1, varname, active)

targets1 |> filter(src == "ak00")
